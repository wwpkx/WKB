package com.wwp.wkb.glide.activity;
...
public class GlideBaseActivity extends Activity {
    ...
    private void initData() {
        ...
        Glide.with(this).load(R.drawable.logo).into(ivGlide2);
        ...
        Glide.with(this).load(gifUrl).placeholder(R.drawable.logo).into(ivGlide4);

        // （5）加载资源gif
        tvGlide5.setText("（5）加载资源gif");
        Glide.with(this).load(R.drawable.loading).asGif().placeholder(R.drawable.logo).into(ivGlide5);

        //（6）加载本地gif
        tvGlide6.setText("（6）加载本地gif");
        String gifPath = Environment.getExternalStorageDirectory() + "/meinv2.jpg";
        File gifFile = new File(gifPath);
        Glide.with(this).load(gifFile).placeholder(R.drawable.logo).into(ivGlide6);

        //（7）加载本地小视频和快照
        tvGlide7.setText("（7）加载本地小视频和快照");
        String videoPath = Environment.getExternalStorageDirectory() + "/video.mp4";
        File videoFile = new File(videoPath);
        Glide.with(this).load(Uri.fromFile(videoFile)).placeholder(R.drawable.logo).into(ivGlide7);
        ...
        Glide.with(this).load(new File(urlPath)).thumbnail(0.1f).centerCrop().placeholder(R.drawable.logo).into(ivGlide8);
        ...
        Glide.with(this).load(Uri.fromFile(videoFile)).thumbnail(thumbnailRequest).centerCrop().placeholder(R.drawable.logo).into(ivGlide9);
    }
}
